...
class PulidModel(nn.Module):
    def __init__(self, model, version="v1.0"):
        ...
        if version == "v1.1":
            self.image_proj_model = IDFormer()
            self.image_proj_model.load_state_dict(model["id_adapter"])
            converted = convert_v11_attn_keys(model["id_adapter_attn_layers"])
            self.ip_layers = To_KV(converted)
        else:
...
class PulidModelLoader:
    ...
    def load_model(self, pulid_file):
        ...
        # Auto-detect version by checking key structure
        if any(k.startswith("id_adapter.") for k in model.keys()):
            version = "v1.1"
            st_model = {"id_adapter": {}, "id_adapter_attn_layers": {}}
            for key in model.keys():
                if key.startswith("id_adapter."):
                    st_model["id_adapter"][key.replace("id_adapter.", "")] = model[key]
                elif key.startswith("id_adapter_attn_layers."):
                    st_model["id_adapter_attn_layers"][key.replace("id_adapter_attn_layers.", "")] = model[key]
            model = st_model
        else:
            version = "v1.0"
        ...
        model = PulidModel(model, version=version)
        print(f"PuLID model loaded: {version}")
...
class ApplyPulid:
    ...
    def apply_pulid(self, model, pulid, eva_clip, face_analysis, image, weight, start_at, end_at, method=None, noise=0.0, fidelity=None, projection=None, attn_mask=None):
        ...
        # Auto-adjust defaults based on PuLID version
        is_v11 = getattr(pulid, 'version', 'v1.0') == 'v1.1'

        if method is not None:
            if method == "fidelity" or projection == "ortho_v2":
                num_zero = 20 if is_v11 else 8
                ortho = False
                ortho_v2 = True
        ...
        # For v1.1 multi-reference: collect per-image embeddings separately
        id_cond_list = []
        id_vit_hidden_list = []
        ...
        for i in range(image.shape[0]):
            ...
            if is_v11:
                # For v1.1: collect for native multi-reference handling
                id_cond_list.append(id_cond)
                id_vit_hidden_list.append(id_vit_hidden)
            else:
                # For v1.0: generate embeddings per image, then average
                if noise == 0:
        ...
        if is_v11:
            ...
            # v1.1 native multi-reference: stack id_cond, concat vit hidden states
            id_cond_stacked = torch.stack(id_cond_list, dim=1)  # [1, N, 1280]

            # Merge vit hidden states: concat along sequence dim
            id_vit_hidden_merged = id_vit_hidden_list[0]
            for i in range(1, len(id_vit_hidden_list)):
                for j, x in enumerate(id_vit_hidden_list[i]):
                    id_vit_hidden_merged[j] = torch.cat([id_vit_hidden_merged[j], x], dim=1)
            ...
            for layer_idx in range(len(id_vit_hidden_list[0])):
                if noise == 0:
                    ...
                    id_vit_hidden_uncond.append(torch.rand_like(id_vit_hidden_list[0][layer_idx]) * noise)

            cond_embeds = pulid_model.get_image_embeds(id_cond_stacked, id_vit_hidden_merged)
            uncond_embeds = pulid_model.get_image_embeds(id_uncond, id_vit_hidden_uncond)
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "fidelity": ("INT", {"default": 20, "min": 0, "max": 32, "step": 1 }),
